...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    locationHostWhitelist: [],      // Requests to these locations will be allowed.
    locationHostBlacklist: [],      // Requests to these locations will be blocked.
  ...
  return function(req, res) {
    ...
    if (location.host !== 'proxy') {
      showUsage(corsAnywhere.helpFile, cors_headers, res);
      return;
    }

    location = parseURL(decodeURIComponent(location.query.split('=')[1]));
    ...
    if (corsAnywhere.locationHostBlacklist.length && corsAnywhere.locationHostBlacklist.indexOf(location.host) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The URL "' + location.href + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (corsAnywhere.locationHostWhitelist.length && corsAnywhere.locationHostWhitelist.indexOf(location.host) === -1) {
        res.writeHead(403, 'Forbidden', cors_headers);
        res.end('The URL "' + location.href + '" was not whitelisted by the operator of this proxy.');
        return;
    }
